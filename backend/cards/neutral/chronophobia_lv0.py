"""Chronophobia — Neutral Treachery, Basic Weakness.
显现：放置入你的威胁区域。
强制 - 当你的回合结束时：受到1点直接恐惧。
[action][action]：丢弃时间恐惧症。
"""
...
class Chronophobia(CardImplementation):
    ...
    def revelation(self, ctx):
        if ctx.extra.get("card_id") != "chronophobia_lv0":
            return
        inv = ctx.game_state.get_investigator(ctx.investigator_id)
        if inv is None:
            return
        if "chronophobia_lv0" in inv.hand:
            inv.hand.remove("chronophobia_lv0")
    ...
    def turn_end_horror(self, ctx):
        ...
        if inv is None:
            return
        if self._find_chronophobia(ctx.game_state, inv) is None:
            return
    ...
    def activate_discard(self, game_state, investigator_id) -> bool:
        ...
        if inv is None:
            return False
        inst = self._find_chronophobia(game_state, inv)
        if inst is None:
            return False
        inv.threat_area.remove(inst.instance_id)
        game_state.cards_in_play.pop(inst.instance_id, None)
        inv.discard.append("chronophobia_lv0")
        return True

    @staticmethod
    def _find_chronophobia(game_state, inv):
        for inst_id in inv.threat_area:
            inst = game_state.get_card_instance(inst_id)
            if inst is not None and inst.card_id == "chronophobia_lv0":
                return inst
        return None
