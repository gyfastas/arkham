"""On the Hunt (Level 0) — Guardian Event. (03263)
快速。在神话阶段你将要抽取一张遭遇卡时打出。
改为在遭遇牌堆顶上9张卡牌中查找一个敌人，生成该敌人并与它交战
（代替原有的生成地点），并混洗遭遇牌堆。如果不能，抽取遭遇牌堆顶的1张牌。

简化说明：
- 从手牌中自动触发（官方为玩家选择时机）：神话阶段轮到你抽遭遇卡、
  且牌堆顶9张内有敌人时，若手牌中有本卡则自动打出；找不到敌人则不打出，
  正常抽牌（官方"如果不能，抽牌堆顶1张"即正常抽取流程）。
- 生成的敌人直接与你交战（代替其正常生成地点），经事件总线发出
  ENEMY_ENGAGED。
- 引擎偏差：phase_mythos 在发出 ENCOUNTER_CARD_DRAWN 前已把牌顶牌弹出，
  且事件后无条件放入遭遇弃牌堆；本卡取消（ctx.cancel）后续结算监听，
  但该牌顶牌仍进入遭遇弃牌堆（官方为从未抽离牌堆）。完整"将要抽取"
  拦截窗口需要引擎支持，见报告。
"""
...
class OnTheHunt(CardImplementation):
    card_id = "on_the_hunt_lv0"
    ...
    def hunt(self, ctx):
        ...
        enemy_card_id = None
        for card_id in scenario.encounter_deck[:9]:
