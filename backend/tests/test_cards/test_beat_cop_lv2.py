"""Tests for Beat Cop (Level 2)."""

import pytest
from backend.cards.guardian.beat_cop_lv2 import BeatCopLv2
...
def game():
    ...
    beat_cop_data = CardData(
        id="beat_cop_lv2", name="Beat Cop", name_cn="巡警",
    ...
    g.card_registry.register_class(BeatCopLv2)
    return g


def _equip_beat_cop_lv2(game):
    inv = game.state.get_investigator("inv1")
    instance_id = game.state.next_instance_id()
    card_inst = CardInstance(
        instance_id=instance_id, card_id="beat_cop_lv2",
        owner_id="inv1", controller_id="inv1",
        slot_used=[SlotType.ALLY],
    )
    game.state.cards_in_play[instance_id] = card_inst
    inv.play_area.append(instance_id)
    game.card_registry.activate_card("beat_cop_lv2", instance_id, game.event_bus)
    return instance_id


class TestBeatCopLv2:
    def test_card_registered(self, game):
        assert "beat_cop_lv2" in game.card_registry.registered_cards

    def test_combat_bonus(self, game):
        """Beat Cop (2) provides +1 combat during fight."""
        _equip_beat_cop_lv2(game)
        _spawn_enemy(game)
        ...
        # Combat 3 + 1 beat cop lv2 = 4 vs fight 3 -> success
